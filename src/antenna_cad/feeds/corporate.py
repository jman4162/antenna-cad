...
class _TreeBuilder:
    ...
    def __init__(self, patch: RectangularPatch, lattice: ArrayLattice, y_bottom: float) -> None:
    ...
    def _element_arm(self, junction: tuple[float, float], col: int, row: int) -> None:
        ...
        if ey > y:  # element above: fed from below, normal orientation
            length = self._run([(x, y), (x, ey - edge)], self.w50)
            self.mirrored[(col, row)] = False
        else:  # element below: fed from above, mirrored + half-wave trombone
            y_to = ey + edge
            span = y - y_to
            if span <= self.w50 * 3:
                raise FeedSynthesisError(
                    f"no room for the phase trombone between y={y:.2f} and the "
                    f"mirrored element edge at y={y_to:.2f}"
                )
            y_mid1 = y - span * 0.35
            y_mid2 = y - span * 0.65
            length = self._run(
                [
                    (x, y),
                    (x, y_mid1),
                    (x + self.jog_50, y_mid1),
                    (x + self.jog_50, y_mid2),
                    (x, y_mid2),
                    (x, y_to),
                ],
                self.w50,
            )
            self.mirrored[(col, row)] = True
    ...
    def build(self) -> tuple[float, float]:
        ...
        y_first = -(self.patch.feed_edge_offset() + max(2.0, 3 * self.w50)) if ny == 1 else 0.0
...
def build_corporate_feed(
    patch: RectangularPatch, lattice: ArrayLattice, y_bottom: float
) -> CorporateFeed:
    ...
    """
    builder = _TreeBuilder(patch, lattice, y_bottom)
    port = builder.build()
